...
class SupplyPoint:
	...
	func _init(new_max_level := 100, new_level := 0, new_demand := 50,
...
func _process(delta):
	...
	if counter > 1:
		...
		if home.stock_level < home.demand_level:
			var customer_vehicle = TransitVehicle.new()
			# Vehicle checks for how much is needed then collects and delivers
			customer_vehicle.collect(min(home.demand_level / 2, 
				home.max_stock_level - home.stock_level), warehouse)
		...
		if warehouse.stock_level < warehouse.demand_level:
			# Trucks are biggers than cars
			var delivery_truck = TransitVehicle.new(100)
			# Vehicle checks for how much is needed then collects and delivers
			delivery_truck.collect(min(warehouse.demand_level,
				warehouse.max_stock_level - warehouse.stock_level), 
				manufacturer)
			delivery_truck.deliver(warehouse)

		# Print the current stock and demand for each SupplyPoint for debugging
		print("Home stock: ", home.stock_level, 
			", Home demand: ", home.demand_level)
		print("Warehouse stock: ", warehouse.stock_level, 
			", Warehouse demand: ", warehouse.demand_level)
		print("Manufacturer stock: ", manufacturer.stock_level, 
			", Manufacturer demand: ", manufacturer.demand_level)
